...
class SolicitudController extends Controller
{
    ...
    {   
        ...
        $traerSolicitudes = $this->traerSolicitudes($estado);

        return View('solicitudes/solicitudes')
        ->with('titulo', $traerSolicitudes['titulo'])
        ->with('Solicitudes', $traerSolicitudes['Solicitudes'])
        ->with('estado_letra', $estado);
    }

    public function traerSolicitudes($estado)
    {   
        ...
        if (Auth::user()->rol_de_usuario_id == 3) {
            ...
            if($estado == 'p') {
                $titulo .= ' (Pendientes)';
                $Solicitudes = Solicitud::whereRaw('((fecha_de_firma_de_solicitud IS NOT NULL AND sino_aprobado_administracion IS NULL AND sino_aprobado_contrato IS NULL) or (sino_aprobado_administracion = "NO" AND sino_aprobado_solicitar_revision = "SI" AND sino_aprobado_contrato IS NULL))')
                    ->where('user_id', Auth::user()->id)
                    ->get();                
            }
            if($estado == 'r') {
            ...
            if($estado == 'd') {
                $titulo .= ' (Desprobadas)';
                $Solicitudes = Solicitud::whereRaw('(((sino_aprobado_administracion = "NO" OR sino_aprobado_garantes = "NO") AND sino_aprobado_solicitar_revision = "NO") OR (sino_aprobado_contrato = "NO"))')
                    ->where('user_id', Auth::user()->id)
                    ->get();                
            }
            if($estado == 'f') {
    ...
    {   
        ...
        // RECALCULO EL PRECIO
        //$Solicitud->valor_total = $this->recalcularPrecioTotal($solicitud_id);


        
        $Solicitud->save();

        $precio_total_minimo = $this->recalcularPrecioTotal($solicitud_id);
        ...
        $Lista_de_precio = Lista_de_precio::where('zona_id', $Solicitud->user->Sucursal->Zona->id)->whereRaw('id in (select p.lista_de_precio_id FROM precios p WHERE p.modelo_id = '.$Solicitud->Modelo->id.')')->get();
        ...
        $Vendedores = Vendedor::where('sucursal_id', $Solicitud->user->Sucursal->id)->get();
        ...
        foreach ($Vendedores as $Vendedor) { 
            $Vendedores_union[] = [
                'id' => $Vendedor['id'],
                'nombre' => $Vendedor[$nombre_del_campo]
                ];
        }

        $valoresSchemaVFG_vendedores = $this->valoresParaSelectEx($Vendedores_union, $nombre_del_campo);
        ...
        $disabled_fecha_de_vencimiento_de_la_solicitud = true;
        $fecha_de_vencimiento_de_la_solicitud = $GenericController->FormatoFecha($Solicitud->fecha_de_vencimiento_de_la_solicitud);
        $disabled_sino_contado = 'true';

        return array('Solicitud' => $Solicitud, 'ComponentesDeModeloSolicitud' => $ComponentesDeModeloSolicitud, 'schemaVFG_lista_de_precios' => $schemaVFG_lista_de_precios, 'parametro_anticipo_minimo' => $parametro_anticipo_minimo, 'valoresSchemaVFG_lista_de_precios' => $valoresSchemaVFG_lista_de_precios, 'valoresSchemaVFG_vendedores' => $valoresSchemaVFG_vendedores, 'Cuotas' => $Cuotas, 'precio_total_minimo' => $precio_total_minimo, 'disabled_fecha_de_vencimiento_de_la_solicitud' => $disabled_fecha_de_vencimiento_de_la_solicitud, 'disabled_sino_contado' => $disabled_sino_contado, 'fecha_de_vencimiento_de_la_solicitud' => $fecha_de_vencimiento_de_la_solicitud);
    }
    ...
    {   
        ...
        ->with('precio_total_minimo', $elementosPS['precio_total_minimo'])
        ->with('disabled_fecha_de_vencimiento_de_la_solicitud', $elementosPS['disabled_fecha_de_vencimiento_de_la_solicitud'])
        ->with('disabled_sino_contado', $elementosPS['disabled_sino_contado'])
        ->with('fecha_de_vencimiento_de_la_solicitud', $elementosPS['fecha_de_vencimiento_de_la_solicitud']);
    }
    ...
    {   
        ...
        if ($sino_aprobado_administracion == 'SI') {
            $Solicitud->sino_aprobado_contrato = NULL;
        ...
        $disabled_primer_vencimiento = 'disabled';
        $disabled_cant_periodo = 'disabled';
        ...
        ->with('ComponentesDeModeloSolicitud', $elementosPS['ComponentesDeModeloSolicitud'])
        ->with('precio_total_minimo', $elementosPS['precio_total_minimo'])
        ->with('disabled_primer_vencimiento', $disabled_primer_vencimiento)
        ->with('disabled_cant_periodo', $disabled_primer_vencimiento)
        ->with('disabled_periodo', $disabled_primer_vencimiento)
        ->with('disabled_fecha_de_vencimiento_de_la_solicitud', $elementosPS['disabled_fecha_de_vencimiento_de_la_solicitud'])
        ->with('disabled_sino_contado', $elementosPS['disabled_sino_contado'])
        ->with('fecha_de_vencimiento_de_la_solicitud', $elementosPS['fecha_de_vencimiento_de_la_solicitud'])
        ->with('mensaje', $mensaje);
    ...
    {   
        ...
        if ($sino_aprobado_garantes == 'SI') {
            ...
            $now = new \DateTime();      
            $Solicitud->fecha_de_aprobacion_garantes = $now->format('Y-m-d H:i:s');
            $mensaje = 'Garantes Aprobados';
        }
        else {
            $Solicitud->observaciones_aprobado_garantes = $_POST['observaciones_aprobado_garantes'];
            $mensaje = 'Garantes Desaprobados';
        }
        $Solicitud->save();
    ...
    {   
        ...
        if ($sino_aprobado_finalizada == 'SI') {
            ...
            $now = new \DateTime();      
            $Solicitud->fecha_de_finalizacion = $now->format('Y-m-d H:i:s');
        }
        else {
            $Solicitud->fecha_de_finalizacion = NULL;
        }
        $Solicitud->save();
    ...
    public function listarClientesParaSeleccion($solicitud_id = null)
    {
        $gen_modelo = 'Cliente';
        $gen_opcion = 0;
        ...
        if ($solicitud_id > 0) {
            $Solicitud = Solicitud::find($solicitud_id);
            $acciones_extra = array('Seleccionar,fa fa-hand-pointer-o,Solicitudes/solicitud/cambiar-cliente/'.$solicitud_id);
            $gen_filas = Cliente::where('sucursal_id', Auth::user()->sucursal_id)
            ->where('id', '<>', $Solicitud->cliente_id)
            ->get();
        }
        else {
            $acciones_extra = array('Seleccionar,fa fa-hand-pointer-o,Solicitudes/crear/elegir-cliente');
            $gen_filas = Cliente::where('sucursal_id', Auth::user()->sucursal_id)->get();
        }

        //$gen_seteo['filtros_por_campo'] = array('sucursal_id' => Auth::user()->sucursal_id);
        $gen_seteo['gen_url_siguiente'] = 'back';

        $gen_campos_a_ocultar = array('empresa_id', 'tipo_de_documento_id', 'domicilio', 'provincia', 'email_correo', 'pais', 'observaciones', 'zona_local_id', 'sucursal_id', 'user_id', 'id_de_importacion');
    ...
    {   
        ...
        $Lista_de_precio = Lista_de_precio::where('zona_id', Auth::user()->Sucursal->Zona->id)->whereRaw('id in (select p.lista_de_precio_id FROM precios p WHERE p.modelo_id = '.$Solicitud->Modelo->id.')')->get();
        $nombre_del_campo = 'lista_de_precio';
        $valoresSchemaVFG_lista_de_precios = $this->valoresParaSelectEx($Lista_de_precio, $nombre_del_campo);
        ...
        //dd($valoresSchemaVFG_lista_de_precios);

        $valoresSchemaVFG_vendedores = $this->valoresParaSelectEx($Vendedores_union, $nombre_del_campo);
        
        $mensaje = '';
        if ($valoresSchemaVFG_lista_de_precios == '') {
            $mensaje['error'] = true;
            $mensaje['detalle'] = 'Este modelo de vivienda no esta cargado en una lista de precios, comuniquese con el supervisor para corregir esto';
        ...
        if ($Solicitud->fecha_de_vencimiento_de_la_solicitud <> '') {
            $fecha_de_vencimiento_de_la_solicitud = $GenericController->FormatoFecha($Solicitud->fecha_de_vencimiento_de_la_solicitud);
            $disabled_fecha_de_vencimiento_de_la_solicitud = false;
        }
        else {
            $fecha_de_vencimiento_de_la_solicitud = date("d/m/Y",strtotime(date("d-m-Y")."+ 1 months"));
            $disabled_fecha_de_vencimiento_de_la_solicitud = true;
        }
        ...
        ->with('pasos_info', $pasos_info)
        ->with('mensaje', $mensaje)               
        ->with('disabled_sino_contado', $disabled_sino_contado)             
        ->with('disabled_fecha_de_vencimiento_de_la_solicitud', $disabled_fecha_de_vencimiento_de_la_solicitud)             
        ->with('fecha_de_vencimiento_de_la_solicitud', $fecha_de_vencimiento_de_la_solicitud);               
    }
    ...
    public function traerValoresPrecio() {
        ...
        $forma_de_pago = $Precio->Lista_de_precio->forma_de_pago_id;
    ...
    public function recalcularPrecioTotal($solicitud_id) {
        $Solicitud = Solicitud::find($solicitud_id);
        $lista_de_precio_id = $Solicitud->lista_de_precio_id;
        $modelo_id = $Solicitud->modelo_id;        
        ...
        //$Solicitud->valor_total = $valor_total;
    ...
    public function imprimirSolicitud($solicitud_id) {
        ...
        $valor_total_centavos_x_100 = intval(($valor_total - $valor_total_entero) * 100);
        $valor_total_formateado = $gCon->formatoNumero($valor_total, 'decimal');
        $valor_total_a_texto = $fcx->functionNumeroALetras($valor_total_entero);
        ...
        $anticipo_centavos_x_100 = intval(($anticipo - $anticipo_entero) * 100);
        $anticipo_formateado = $gCon->formatoNumero($anticipo, 'decimal');
        $anticipo_a_texto = $fcx->functionNumeroALetras($anticipo_entero);
        ...
        $resto_centavos_x_100 = intval(($resto - $resto_entero) * 100);
        $resto_formateado = $gCon->formatoNumero($resto, 'decimal');
        $resto_a_texto = $fcx->functionNumeroALetras($resto_entero);
        ...
        $sustituciones[6] = $Solicitud->Cliente->localidad->localidad;
        $sustituciones[7] = $Solicitud->Cliente->localidad->provincia->provincia;
    ...
    public function imprimirContrato($solicitud_id) {
        ...
        //ANTICIPO + PAGADO
        $pagado_mas_anticipo = $Solicitud->anticipo + $Solicitud->pagado;
        $pagado_mas_anticipo_entero = intval($pagado_mas_anticipo);
        $pagado_mas_anticipo_centavos_x_100 = intval(($pagado_mas_anticipo - $pagado_mas_anticipo_entero) * 100);
        $pagado_mas_anticipo_formateado = $gCon->formatoNumero($pagado_mas_anticipo, 'decimal');
        $pagado_mas_anticipo_a_texto = $fcx->functionNumeroALetras($pagado_mas_anticipo_entero);
        $solicitud_pagado_mas_anticipo = "$moneda $pagado_mas_anticipo_a_texto y $pagado_mas_anticipo_centavos_x_100 / 100 centavos ($pagado_mas_anticipo_formateado)";

        //SALDO
        $saldo = $valor_total - $anticipo;
        $saldo_entero = intval($saldo);
        $saldo_centavos_x_100 = intval(($saldo - $saldo_entero) * 100);
        $saldo_formateado = $gCon->formatoNumero($saldo, 'decimal');
        $saldo_a_texto = $fcx->functionNumeroALetras($saldo_entero);
        $solicitud_saldo = "$ $saldo_formateado ($moneda $saldo_a_texto y $saldo_centavos_x_100 / 100 centavos)";

        //SALDO50 MENOS ANTICIPO
        $saldo50_menos_anticipo = ($valor_total/2) - $pagado_mas_anticipo ;
        if ($saldo50_menos_anticipo < 0) {
            $saldo50_menos_anticipo = $saldo50_menos_anticipo*-1;
        }
        $saldo50_menos_anticipo_entero = intval($saldo50_menos_anticipo);
        $saldo50_menos_anticipo_centavos_x_100 = intval(($saldo50_menos_anticipo - $saldo50_menos_anticipo_entero) * 100);
        $saldo50_menos_anticipo_formateado = $gCon->formatoNumero($saldo50_menos_anticipo, 'decimal');
        //dd($saldo50_menos_anticipo);
        $saldo50_menos_anticipo_a_texto = $fcx->functionNumeroALetras($saldo50_menos_anticipo_entero);
        ...
        $saldo50_centavos_x_100 = intval(($saldo50 - $saldo50_entero) * 100);
        $saldo50_formateado = $gCon->formatoNumero($saldo50, 'decimal');
        $saldo50_a_texto = $fcx->functionNumeroALetras($saldo50_entero);
        ...
        if ($Solicitud->cuotas_anticipo > 0) {
            $solicitud_cuotas_anticipo = $Solicitud->cuotas_anticipo;
        }
        else {
            $solicitud_cuotas_anticipo = 1;
        }        

        if ($Solicitud->pagado > 0 and $Solicitud->anticipo > $Solicitud->pagado) {
            $anticipo_menos_pagado = $Solicitud->anticipo - $Solicitud->pagado;
        }
        else {
            $anticipo_menos_pagado = $Solicitud->anticipo;            
        }
        $monto_cuotas_anticipo = $anticipo_menos_pagado / $solicitud_cuotas_anticipo;
        $monto_cuotas_anticipo_entero = intval($monto_cuotas_anticipo);
        $monto_cuotas_anticipo_centavos_x_100 = intval(($monto_cuotas_anticipo - $monto_cuotas_anticipo_entero) * 100);
        $monto_cuotas_anticipo_formateado = $gCon->formatoNumero($monto_cuotas_anticipo, 'decimal');
        $monto_cuotas_anticipo_a_texto = $fcx->functionNumeroALetras($monto_cuotas_anticipo_entero);
        ...
        $monto_cuotas_contrato = $solicitud_monto_cuotas_contrato;
        $monto_cuotas_contrato_entero = intval($monto_cuotas_contrato);
        $monto_cuotas_contrato_centavos_x_100 = intval(($monto_cuotas_contrato - $monto_cuotas_contrato_entero) * 100);
        $monto_cuotas_contrato_formateado = $gCon->formatoNumero($monto_cuotas_contrato, 'decimal');
        $monto_cuotas_contrato_a_texto = $fcx->functionNumeroALetras($monto_cuotas_contrato_entero);
        ...
        //MONTO PAGADO
        $monto_pagado = $Solicitud->pagado;
        $monto_pagado_entero = intval($monto_pagado);
        $monto_pagado_centavos_x_100 = intval(($monto_pagado - $monto_pagado_entero) * 100);
        $monto_pagado_formateado = $gCon->formatoNumero($monto_pagado, 'decimal');
        $monto_pagado_a_texto = $fcx->functionNumeroALetras($monto_pagado_entero);
        $solicitud_monto_pagado = "$ $monto_pagado_formateado ($moneda $monto_pagado_a_texto y $monto_pagado_centavos_x_100 / 100 centavos)";


        //RESTO ANTICIPO
        $texto_resto_anticipo = '';
        if ($monto_pagado < $anticipo and $Solicitud->sino_contado <> 'SI') {
            $cant_cuotas_anticipo = $Solicitud->cuotas_anticipo;
            $resto_de_anticipo = $anticipo-$monto_pagado;
            $monto_cuotas_anticipo = $anticipo/$cant_cuotas_anticipo;
            $modulo_de_resto_cutoas_anticipo = $resto_de_anticipo % $monto_cuotas_anticipo;
            $resto_de_anticipo_cant_cuotas = '';
            $cant_cuotas_anticipo_restantes = '';
            if ($modulo_de_resto_cutoas_anticipo == 0) {
                $cant_cuotas_anticipo_restantes = intval($resto_de_anticipo/$monto_cuotas_anticipo);
            }
            ...
            $texto_resto_anticipo = "El resto del anticipo EL/LA LOCATARIO/A se obliga a pagarlo en $cant_cuotas_anticipo_restantes cuotas de <strong>$solicitud_monto_cuotas_anticipo</strong>.";
        }
        ...
        $patrones[25] = '/<ol>/';
        $patrones[26] = '/solicitud_monto_pagado/';
        $patrones[27] = '/texto_resto_anticipo/';
        $patrones[28] = '/solicitud_pagado_mas_anticipo/';
        $patrones[29] = '/sucursal_provincia/';
        $sustituciones = array();
        $sustituciones[0] = $Solicitud->cliente_id;
        ...
        $sustituciones[19] = '<strong>'.$solicitud_cuotas_anticipo.'</strong>';
        ...
        $sustituciones[25] = '<ol type="A">';
        $sustituciones[26] = '<strong>'.$solicitud_monto_pagado.'</strong>';
        $sustituciones[27] = $texto_resto_anticipo;
        $sustituciones[28] = '<strong>'.$solicitud_pagado_mas_anticipo.'</strong>';
        $sustituciones[29] = '<strong>'.$Solicitud->sucursal->localidad->provincia->provincia.'</strong>';
        ...
        $encabezado .= '</head>';
        $encabezado .= '<body>';
        $encabezado .= '<header><div style="float: left"><img src="'.env('PATH_PUBLIC').'img/logo.jpg" style="width: 200px"></div><div style="float: right; margin-right: 100px; margin-top: 80px; font-size: 15px; font-weight: bold">CONTRATO: '.$Solicitud->sucursal->id.'-'.$Solicitud->Cliente->id_externo.'</div></header>';
        //$encabezado .= '<header><p style="text-align: right"><img src="'.env('PATH_PUBLIC').'img/logo.jpg" style="width: 200px"></p></header><footer>info pie</footer>';
    ...
    public function imprimirAdquiriente($solicitud_id) {
        ...
        $encabezado .= '<head>';
        $encabezado .= '  <style>';
        $encabezado .= '    @page { margin-top: 80px; margin-left: 80px; margin-right: 80px; font-family: Arial, Helvetica, sans-serif; font-size: 13px}';
        $encabezado .= '    header { position: fixed; top: -60px; left: 0px; right: 0px; height: 100px; margin-top: 70px }';
        $encabezado .= '    footer { position: fixed; bottom: -60px; left: 0px; right: 0px; height: 50px; }';
        $encabezado .= '    p { text-align: justify; }';
        //$encabezado .= '    p { page-break-after: always; }';
    ...
    public function imprimirCaracteristicasTecnicas($solicitud_id) {
        ...
        $encabezado .= '<head>';
        $encabezado .= '  <style>';
        $encabezado .= '    @page { margin-top: 80px; margin-left: 110px; margin-right: 110px; font-family: Arial, Helvetica, sans-serif; font-size: 13px}';
        $encabezado .= '    header { position: fixed; top: -60px; left: 0px; right: 0px; height: 100px; margin-top: 70px }';
        $encabezado .= '    footer { position: fixed; bottom: -60px; left: 0px; right: 0px; height: 50px; }';
        $encabezado .= '    p { text-align: justify; }';
        //$encabezado .= '    p { page-break-after: always; }';
    ...
    public function imprimirAnexo5($solicitud_id) {

        $Solicitud = Solicitud::find($solicitud_id);
        $Reporte = Reporte::find(20);
    ...
    public function imprimirGarantes($solicitud_id) {
        ...
        $patrones[7] = '/cliente_provincia/';
        $sustituciones = array();
        $sustituciones[0] = $Solicitud->id;
        $sustituciones[1] = '<strong>'.$Solicitud->Cliente->nombre.'</strong>';
        $sustituciones[2] = '<strong>'.$Solicitud->Cliente->apellido.'</strong>';
        $sustituciones[3] = '<strong>'.$Solicitud->Cliente->Tipo_de_documento->tipo_de_documento.'</strong>';
        $sustituciones[4] = '<strong>'.$gCon->formatoNumero($Solicitud->Cliente->nro_de_documento, 'entero').'</strong>';
        $sustituciones[5] = '<strong>'.$Solicitud->Cliente->domicilio.'</strong>';
        $sustituciones[6] = '<strong>'.$Solicitud->Cliente->localidad->localidad.'</strong>';
        $sustituciones[7] = '<strong>'.$Solicitud->Cliente->localidad->provincia->provincia.'</strong>';
        ...
        foreach ($Garantes as $Garante) {
            ...
            $patrones[4] = '/garante_numero_de_documento/';
            $patrones[5] = '/garante_domicilio/';
            $patrones[6] = '/garante_inmueble/';
            $patrones[7] = '/garante_tomo/';
            $patrones[8] = '/garante_folio/';
            $patrones[9] = '/garante_numero/';
            $patrones[10] = '/garante_departamento/';
            $sustituciones = array();
            ...
            $sustituciones[4] = '<strong>'.$gCon->formatoNumero($Garante->numero_de_documento, 'entero').'</strong>';
            ...
            $sustituciones[9] = '<strong>'.$Garante->numero.'</strong>';
            $sustituciones[10] = '<strong>'.$Garante->departamento.'</strong>';
    ...
    public function cancelarContrato($solicitud_id)
    {   

        $Solicitud = Solicitud::find($solicitud_id);
        $Solicitud->fecha_de_contrato = null;
        $Solicitud->observaciones_contrato = null;
        $Solicitud->sino_aprobado_administracion = null;
        $Solicitud->observaciones_aprobado_administracion = null;

        $Solicitud->save();

        $mensaje = 'El Contrato se ha cancelado, el vendedor puede nuevamente editar la solicitud';
    ...
    public function aprobacionContrato($solicitud_id)
    {   
        $sino_aprobado_contrato = $_POST['sino_aprobado_contrato'];

        $Solicitud = Solicitud::where('id', $solicitud_id)->first();
        $Solicitud->sino_aprobado_contrato = $sino_aprobado_contrato;
        if ($sino_aprobado_contrato == 'SI') {
            $Solicitud->sino_aprobado_finalizada = 'SI';
            $Solicitud->observaciones_aprobado_contrato = NULL;
            $mensaje = 'Contrato Aprobado';
        }
        else {
            $Solicitud->sino_aprobado_finalizada = null;
            $Solicitud->fecha_de_contrato = null;
            $Solicitud->observaciones_contrato = null;
            $Solicitud->sino_aprobado_administracion = 'NO';
            $Solicitud->sino_aprobado_solicitar_revision = NULL;
            $Solicitud->observaciones_aprobado_solicitar_revision = NULL;
            $Solicitud->sino_aprobado_garantes = null;
            $Solicitud->observaciones_aprobado_administracion = null;
            $Solicitud->observaciones_aprobado_garantes = null;

            $Solicitud->observaciones_aprobado_administracion = $_POST['observaciones_aprobado_contrato'];
            $mensaje = 'Contrato Desaprobado';
        }
        $Solicitud->save();
